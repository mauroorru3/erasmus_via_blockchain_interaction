...
import { ChainInfo } from "./module/types/universitychainde/chain_info"
import { ContactInfo } from "./module/types/universitychainde/contact_info"
import { CountersInfo } from "./module/types/universitychainde/counters_info"
...
import { OperationInfo } from "./module/types/universitychainde/operation"
...
import { ErasmusIndexPacketData } from "./module/types/universitychainde/packet"
import { ErasmusIndexPacketAck } from "./module/types/universitychainde/packet"
import { EndErasmusPeriodRequestPacketData } from "./module/types/universitychainde/packet"
import { EndErasmusPeriodRequestPacketAck } from "./module/types/universitychainde/packet"
import { FinalErasmusDataPacketData } from "./module/types/universitychainde/packet"
import { FinalErasmusDataPacketAck } from "./module/types/universitychainde/packet"
import { ExtendErasmusPeriodPacketData } from "./module/types/universitychainde/packet"
import { ExtendErasmusPeriodPacketAck } from "./module/types/universitychainde/packet"
import { ErasmusRestictedDataPacketData } from "./module/types/universitychainde/packet"
import { ErasmusRestictedDataPacketAck } from "./module/types/universitychainde/packet"
import { Params } from "./module/types/universitychainde/params"
import { PersonalInfo } from "./module/types/universitychainde/personal_info"
import { ProfessorsExams } from "./module/types/universitychainde/professors_exams"
import { ResidenceInfo } from "./module/types/universitychainde/residence_info"
import { StoredStudent } from "./module/types/universitychainde/stored_student"
import { StudentInfo } from "./module/types/universitychainde/student_info"
import { TaxesInfo } from "./module/types/universitychainde/taxes_info"
import { TranscriptOfRecords } from "./module/types/universitychainde/transcript_of_records"
import { UniversityInfo } from "./module/types/universitychainde/university_info"


export { ChainInfo, ContactInfo, CountersInfo, ErasmusInfo, ForeignUniversities, OperationInfo, UniversitychaindePacketData, NoData, ErasmusStudentPacketData, ErasmusStudentPacketAck, ErasmusIndexPacketData, ErasmusIndexPacketAck, EndErasmusPeriodRequestPacketData, EndErasmusPeriodRequestPacketAck, FinalErasmusDataPacketData, FinalErasmusDataPacketAck, ExtendErasmusPeriodPacketData, ExtendErasmusPeriodPacketAck, ErasmusRestictedDataPacketData, ErasmusRestictedDataPacketAck, Params, PersonalInfo, ProfessorsExams, ResidenceInfo, StoredStudent, StudentInfo, TaxesInfo, TranscriptOfRecords, UniversityInfo };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						ErasmusRestictedDataPacketData: getStructure(ErasmusRestictedDataPacketData.fromPartial({})),
						ErasmusRestictedDataPacketAck: getStructure(ErasmusRestictedDataPacketAck.fromPartial({})),
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: university_chain_de.universitychainde initialized!')
	...
	gas: "200000" }, memo})
		...
		async sendMsgEndErasmusBeforeDeadline({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgEndErasmusBeforeDeadline(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgEndErasmusBeforeDeadline:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgEndErasmusBeforeDeadline:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async sendMsgExtendErasmus({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgExtendErasmus(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgExtendErasmus:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgExtendErasmus:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgEndErasmusBeforeDeadline({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgEndErasmusBeforeDeadline(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgEndErasmusBeforeDeadline:Create Could not create message: ' + e.message)
				}
			}
		},
		...
		async MsgExtendErasmus({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgExtendErasmus(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgExtendErasmus:Create Could not create message: ' + e.message)
				}
			}
		},
