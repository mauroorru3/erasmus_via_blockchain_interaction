...
func (k Keeper) ClearErasmusCareer(ctx sdk.Context, studentIndex string) (err error) {
	...
	if !found {
		student, err = k.GetStudentByForeignIndex(ctx, studentIndex)
		if err != nil {

			utilfunc.PrintLogs("ClearErasmusCareer - error ErrStudentNotPresent", ctx)
	...
	if student.ErasmusData.ErasmusStudent == "Outgoing" {
		...
		if !found {
			...
			student.Counters.ErrorAcksOrTimeoutsReceived = 0
			student.Counters.AcksReceivedStartErasmus = 0
			student.Counters.RetryNumberOperations = 0
			for i := 0; i < len(student.Counters.PacketsRetriesStartErasmus); i++ {
				student.Counters.PacketsRetriesStartErasmus[i] = 0
			}

			student.Counters.RevertErasmusCareerCompleted = true
...
func (k Keeper) CreateAbortOperationString10Packets(student types.StoredStudent) (abort_op_JSON string, err error) {
	...
	abort_op.ForeignUniversity = student.StudentData.UniversityName
	abort_op.HomeUniversity, _ = utilfunc.GetForeignUniversityName(student)
	abort_op.PacketID = "-4" // Value that identifies the packet related to the abort operation of the last 10 packets of the Start Erasmus operation
...
func (k Keeper) CreateAbortOperationStringV2(studentIndex string, homeUni string, foreignUni string) (abort_op_JSON string, err error) {

	var abort_op utilfunc.AbortOperationPacket

	abort_op.HomeIndex = studentIndex
	abort_op.ForeignIndex = ""
	abort_op.ForeignUniversity = foreignUni
	abort_op.HomeUniversity = homeUni
	abort_op.PacketID = "-1" // Value that identifies the packet related to the abort operation of the Start Erasmus operation
...
func (k Keeper) HandleAbortAckStartErasmus(ctx sdk.Context, studentIndex string) (packetAck types.ErasmusRestictedDataPacketAck, err error) {

	utilfunc.PrintLogs("HandleAbortAckStartErasmus", ctx)
	...
	if err != nil {
		...
		utilfunc.PrintLogs("HandleAbortAckStartErasmus data "+new_data_info, ctx)
		packetAck.ErasmusRestrictedInfo = new_data_info
		return packetAck, err
	}
}

func (k Keeper) HandleAbortAckStartErasmus10Packets(ctx sdk.Context, studentIndex string) (packetAck types.ErasmusRestictedDataPacketAck, err error) {

	utilfunc.PrintLogs("HandleAbortAckStartErasmus10Packets", ctx)
	...
	new_data_info, err := k.CreateAbortOperationString10Packets(storedStudent)
	if err != nil {
		return packetAck, err
	} else {

		utilfunc.PrintLogs("HandleAbortAckStartErasmus10Packets data "+new_data_info, ctx)
		packetAck.ErasmusRestrictedInfo = new_data_info
		return packetAck, err
	}
}

func (k Keeper) HandleAbortAckStartErasmusV2(ctx sdk.Context, studentIndex string, homeUni string, foreignUni string) (packetAck types.ErasmusRestictedDataPacketAck, err error) {

	utilfunc.PrintLogs("HandleAbortAckStartErasmusV2", ctx)

	new_data_info, err := k.CreateAbortOperationStringV2(studentIndex, homeUni, foreignUni)
	if err != nil {
		return packetAck, err
	} else {
...
func (k Keeper) RevertEndErasmus(ctx sdk.Context, studentIndex string) (err error) {
	...
	if !found {
		...
		if err != nil {
			...
			utilfunc.PrintLogs("RevertEndErasmus - error ErrStudentNotPresent", ctx)
			return types.ErrStudentNotPresent
		}
	}
	...
	if student.ErasmusData.ErasmusStudent == "Waiting for updated data from the destination university" {
		...
		if !found {
			...
			start_date, err := time.Parse(utilfunc.DeadlineLayout, erasmusCareer[lenCareer-1].Start_date)
			if err != nil {
				return err
			}

			// I add a few seconds (obviously for testing) so that the Erasmus deadline is postponed and the packet is sent again
			endDate := start_date.Add(time.Duration(600*time.Second) + time.Duration(60*time.Second))
...
func (k Keeper) GetStudentByForeignIndex(ctx sdk.Context, foreign_index string) (student types.StoredStudent, err error) {
	...
	if found {
		...
		return student, types.ErrStudentNotPresent
	}
}
