...
export const AnnualTaxes = {
  encode(message: AnnualTaxes, writer: Writer = Writer.create()): Writer {
    if (message.id !== 0) {
      writer.uint32(8).uint64(message.id);
    }
    if (message.year !== 0) {
      writer.uint32(16).uint64(message.year);
    }
    if (message.amount !== 0) {
      writer.uint32(24).uint64(message.amount);
    }
    if (message.incomeBracket !== 0) {
      writer.uint32(32).uint64(message.incomeBracket);
    }
    if (message.paymentMade === true) {
      writer.uint32(40).bool(message.paymentMade);
    }
    if (message.dateOfPayment !== "") {
      writer.uint32(50).string(message.dateOfPayment);
    }
    return writer;
  },

  decode(input: Reader | Uint8Array, length?: number): AnnualTaxes {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseAnnualTaxes } as AnnualTaxes;
  ...
  fromJSON(object: any): AnnualTaxes {
    const message = { ...baseAnnualTaxes } as AnnualTaxes;
  ...
  toJSON(message: AnnualTaxes): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<AnnualTaxes>): AnnualTaxes {
    const message = { ...baseAnnualTaxes } as AnnualTaxes;
    if (object.id !== undefined && object.id !== null) {
