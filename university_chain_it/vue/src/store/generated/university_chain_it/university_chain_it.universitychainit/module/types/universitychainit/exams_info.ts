...
export const ExamsInfo = {
  encode(message: ExamsInfo, writer: Writer = Writer.create()): Writer {
    if (message.examName !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): ExamsInfo {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseExamsInfo } as ExamsInfo;
  ...
  toJSON(message: ExamsInfo): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<ExamsInfo>): ExamsInfo {
    const message = { ...baseExamsInfo } as ExamsInfo;
    if (object.examName !== undefined && object.examName !== null) {
