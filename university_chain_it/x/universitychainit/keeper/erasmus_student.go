		...
		if err != nil {
			...
			if !found {
				...
				if err != nil {
					...
					err = utilfunc.GetConsumedGas("OnRecvErasmusStudentPacket IT", data.Student.Index, ctx)
					if err != nil {
						return packetAck, err
					} else {
...
func (k Keeper) OnAcknowledgementErasmusStudentPacket(ctx sdk.Context, packet channeltypes.Packet, data types.ErasmusStudentPacketData, ack channeltypes.Acknowledgement) error {
	...
	case *channeltypes.Acknowledgement_Result:
		...
		if err := types.ModuleCdc.UnmarshalJSON(dispatchedAck.Result, &packetAck); err != nil {
			...
			utilfunc.PrintLogs("OnAcknowledgementErasmusStudentPacket cannot unmarshal acknowledgment "+err.Error(), ctx)
		...
		err = utilfunc.GetConsumedGas("OnAcknowledgementErasmusStudentPacket IT", data.Student.Index, ctx)
