...
func networkWithUniversityInfoObjects(t *testing.T, n int) (*network.Network, []types.UniversityInfo) {
	t.Helper()
	...
	for i := 0; i < n; i++ {
		...
		state.UniversityInfoList = append(state.UniversityInfoList, universityInfo)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.UniversityInfoList
}

func TestShowUniversityInfo(t *testing.T) {
	net, objs := networkWithUniversityInfoObjects(t, 2)
...
func TestListUniversityInfo(t *testing.T) {
	net, objs := networkWithUniversityInfoObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListUniversityInfo(), args)
			require.NoError(t, err)
			var resp types.QueryAllUniversityInfoResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
