...
func CmdSendExtendErasmusPeriod() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "send-extend-erasmus-period [src-port] [src-channel] [duration-in-months] [destination-university-name] [foreign-index]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgSendExtendErasmusPeriod(creator, srcPort, srcChannel, timeoutTimestamp, argDurationInMonths, argDestinationUniversityName, argForeignIndex)
